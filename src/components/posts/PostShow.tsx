...
const PostShow: VFC<any> = (id) => {
  ...
  return (
    <div className="w-full lg:w-11/12 lg:mx-auto">
      <h2 className="mt-10 text-4xl text-gray-700 font-bold text-center">
        {post.title}
      </h2>
      ...
      <div className="flex my-4 overflow-x-auto">
        ...
        {genres?.map((genre: string, index: number) => (
          <div
            className="bg-green-200 mx-2 rounded-md p-2 whitespace-nowrap"
            key={index}
          >
            {genre}
          </div>
      ...
      <h3 className="border border-blue-600 text-blue-600 rounded-md text-center p-1 text-lg mx-2 my-2 whitespace-nowrap overflow-x-auto md:text-2xl md:p-2">
        {spot.name}
      </h3>
      <img
        src={image_url}
        className="block mx-auto object-cover w-full"
        alt=""
      />
      ...
      <div className="mt-3 px-2">
        ...
        <div className="pl-2">
          ...
          {spot.web_url ? (
            <a className="text-blue-700 pt-2 block text-lg" href={spot.web_url}>
              {spot.name}
            </a>
          ) : (
